...
exactly one scoped bearer input outside Git. Use `md-drive`; do not use raw
HTTP, `curl`, Google APIs, or any Google credential.

Read first: use `md-drive list`, `md-drive search`, or `md-drive read` before a
mutation. Retain the returned opaque revision. Issue one explicit update with
that revision. On `CONFLICT`, stop, reread, describe the newer state, and wait
for the user's choice. Never silently retry or overwrite. Ask for explicit
user confirmation before archive unless it was already requested.
