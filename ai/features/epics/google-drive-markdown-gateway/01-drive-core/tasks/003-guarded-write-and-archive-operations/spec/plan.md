...
| Writer capabilities are opaque tokens, not virtual dispatch objects. | `MarkdownService` accepts only module-authenticated writer instances and invokes module-owned create/update/move functions. Their private `WeakMap` state revalidates the lease immediately before the raw provider call; concrete writer constructors reject subclasses. Prototype forgery, proxies, or replaced instance methods therefore cannot alter write dispatch. | Adversarial repair: public virtual methods made an `instanceof`-passing subclass a raw-write bypass. |
...
| Update is one conditional mutation; archive is deferred. | Send the supplied exact ETag in `If-Match` for update using the proven `PATCH` form. Map only HTTP 412 to stable `CONFLICT`; do not retry, fall back to unconditional mutation, re-read-and-retry, or turn other failure into conflict. Archive remains disabled pending an atomic destination-topology condition. | Task brief; platform probe; RFC 9110; adversarial repair |
| Ancestor and destination membership have no provider-side precondition. | Until the provider exposes an atomic ancestor-topology condition, permit create and update only for files directly beneath the configured root; nested mutations fail closed before writer dispatch. Archive remains unavailable entirely because an external actor can move the destination folder after resolution. | Adversarial repair: a lease or file ETag alone cannot close an ancestor- or destination-folder move race. |
| Archive is a future move-only operation. | When an atomic destination-topology condition is available, it must require an expected revision, re-verify archive is a root descendant, reject occupied/ambiguous archive leaf, then issue one conditional `addParents`/`removeParents` move. Never trash, delete, remove all parents, or fall back unconditionally. | Handoff; task 001; task brief; adversarial repair |
