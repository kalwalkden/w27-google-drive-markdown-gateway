...
- The shipped clean-environment harness records the current direct-root-only, writes-disabled, archive-disabled profile and blocks before invoking the CLI. The future state machine remains fake-testable only: preflight → list/search/read → create → reread → fresh update → reread → stale update returns `CONFLICT` → reread unchanged state → archive → cleanup verified. Configuration cannot unlock it, and a failed transition cannot be papered over by retrying a mutation.
...
- Archive cleanup targets only the in-memory locator for the generated run file and uses a currently reread revision. A successful response remains inconclusive unless its returned identity and exact generated archive location are verified. An uncertain create preserves only the opaque run reference and fixed manual-cleanup direction. Cleanup failure takes precedence in the final harness outcome.
- Evidence is a strict allowlist and remains external. It may include opaque release/environment/gateway/run identifiers, methods, timestamps, actual platform-control states, the current gateway capability profile, operation outcomes, conflict outcome, cleanup status, and redaction state. It excludes secret values/references, endpoints/URLs/IPs, paths/names, content/digests, Drive IDs, revisions, operation IDs, response bodies, diagnostic strings, logs, and screenshots. Physical ancestry checks and atomic no-replace publication keep repository and failure paths safe.
