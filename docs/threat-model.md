...
| Malformed/oversize input and rate-limit exhaustion | Availability and parser safety; strict schemas, byte limits, request timeout, and per-principal rate/concurrency limits. | HTTP validation, limiter, and timeout tests. | Coordinated distributed abuse can still consume upstream capacity; operator owns capacity response. |
