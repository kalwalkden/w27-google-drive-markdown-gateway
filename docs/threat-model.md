...
Operator / Drive admin -> Terraform, Secret Manager, Drive ACLs, live release evidence (separate)
```

Trust boundaries are: external client to Cloud Run; Work issuer/JWKS and Codex secret mount to verifier; non-secret configuration and secret mounts to runtime; gateway to Google Drive; application policy to the Drive port; configured root/archive topology; and operator-owned Terraform, Secret Manager, Drive ACLs, and live release evidence. Normal tests make no provider, secret, deployment, or telemetry network action.

The product supports bounded, root-confined nested and recursive reads plus revision-checked nested
create, update, and archive when a controlled deployment has composed its server-side writer.
Absent or false deployment write mode remains read-only. `/healthz` is liveness, not Drive
readiness.

## Assets, goals, and actors

Protected assets are document confidentiality; document and revision integrity; root/archive confinement; bounded availability; Work, Codex, and Google credentials; runtime identity/configuration; deployment-owned write mode and release evidence; and telemetry integrity/confidentiality. Actors are an unauthenticated or hostile client, Work MCP client, Codex client, runtime service account, Google Drive and JWKS providers, Secret Manager, deployment/Drive administrator, and log/metric destination.
...
| Traversal, ID escape, shortcuts, duplicates, or moved topology | Root confinement; opaque-ID validation, guarded ports, bounded nested/recursive topology checks, duplicate detection, and verified archive moves. | Domain, service, and Drive-adapter tests. | File `If-Match` serializes the target only. Complete pre/post snapshots detect ordinary target/ancestor changes, but no Drive transaction proves an ancestor was never moved out and back between checks; trusted Drive administration and change control own that residual. |
| Stale overwrite, timeout duplicate/retry, archive-delete, or write-authority bypass | Integrity; revision-checked mutations, default-disabled deployment mode, archive-only cleanup, and no permanent deletion. | Service and HTTP conflict/timeout/write tests. | A timeout cannot prove a dispatched mutation did not apply. `OUTCOME_UNKNOWN` and transport timeout require reread/reconciliation, never retry or rollback; deployment review and release evidence do not grant runtime authority. |
